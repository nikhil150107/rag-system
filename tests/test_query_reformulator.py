"""Unit tests for QueryReformulator service."""
import pytest
...
def test_standalone_question_reformulation():
    ...
    reformulator = QueryReformulator(openai_client=mock_openai, max_history_turns=5)
    history = [
...
def test_reformulator_failure_falls_back_safely():
    """Test 5: On OpenAI error or timeout, safely falls back to original question."""
    mock_openai = MagicMock()
    mock_openai.chat.completions.create.side_effect = Exception("OpenAI API rate limit / timeout error")
...
def test_history_limit_truncation():
    ...
    reformulator = QueryReformulator(openai_client=mock_openai, max_history_turns=2)
    ...
    called_messages = mock_openai.chat.completions.create.call_args[1]["messages"]
...
def test_sanitize_history_filters_invalid_roles_and_empty_content():
    ...
    reformulator = QueryReformulator(openai_client=None, max_history_turns=5)
    dirty_history = [
